...
SeVisualAPR::~SeVisualAPR()
{
	...
	Safe_DeleteVec(m_pInitCuttingPlanePoint);
...
void SeVisualAPR::SetDcmArray(CDcmPicArray* pArray)
{
	m_pDcmArray = pArray;
	CString	csMinValue;
	// pElement can not get right point, the m_sInitValue always equals 0; you can fix it if you like.
...
CDcmPic* SeVisualAPR::GetAPRImage(double dRotateMatrix[16], double dCenterX, double dCenterY, double dCenterZ, bool bMipDownSampling/*=FALSE*/)
{
...
//	LeaveCriticalSection(&g_cs);
	...
	pDcm->SetPixelData((BYTE*)pDataCpy, m_nCuttingPlaneWidth, m_nCuttingPlaneHeight); //注意：这里的数据区要求在外部copy，而不是单纯的指针
...
// 		}
// 	}
	memcpy(m_pCuttingPlanePoint, m_pInitCuttingPlanePoint, sizeof(Vector3D) * m_nCuttingPlaneWidth * m_nCuttingPlaneHeight);
}
...
void SeVisualAPR::Compute2DAPRLine()
{
	...
	if (fabs(fPoint1X) == 0.0 && fabs(fPoint2X) == 0.0 && fabs(fPoint3X) == 0.0) //割平面与坐标系三平面平行
	{
		dAngleXOY = 90.0;
		dAngleXOZ = 90.0;
		m_bYOZParallel = true;
	...
	else
	{
		if ((fabs(fPoint1Z)/(m_nCuttingPlaneWidth/1.414) <= 0.0849) && 
			...
			(fabs(fPoint3Z)/(m_nCuttingPlaneWidth/1.414) <= 0.0849))  //近似于平行某一平面时因为斜率变化敏感，不显示直线
		{
			m_bXOYParallel = true;
		}
		else
...
//	EnterCriticalSection(&g_cs);
	memcpy(dModelViewCpy, m_dRotateMatrix, sizeof(double)*16);   //用线程锁实现线程互斥，复制旋转矩阵，防止主线程与子线程读写冲突
//	LeaveCriticalSection(&g_cs);
	////////////////////////////////////////////////更新初始化割平面//////////////////////////////////////////
	...
	if (m_nMIPPieceNum == 1)
	{
 		for (i = 0; i < m_nCuttingPlaneHeight; i++)
		{
			...
			for (int j = 0; j < m_nCuttingPlaneWidth; j++)
			{
				...
				if (dX < -m_nOriImageWidth/2 || dX >= m_nOriImageWidth/2-1 || dY < -m_nOriImageHeight/2 || dY >= m_nOriImageHeight/2-1 || dZ < -m_nOriImagePiece/2 || dZ >= m_nOriImagePiece/2-1)  //超出体数据范围不做计算
...
// 	CString str;// = "111";
// 	str.Format("中心坐标：%.2f %.2f %.2f,初始化矩阵用时：%d, 矩阵转化用时：%d,未知函数用时 %d, 内存读写用时：%d, 等待绘图用时：%d, 总用时：%d\n", m_dCenterX, m_dCenterY, m_dCenterZ, start1 - start0, end0 - start1, end1 - end0, end2 - end1, end3 - end2, end3 - start0);
...
{    
// 	HINSTANCE hcudaDll=LoadLibrary(__T("SeGeneralMIP.dll"));//动态地加载smooth.dll
// 	if (hcudaDll)
// 	{
// 		MIPFUNC14 dllFun=(MIPFUNC14)GetProcAddress(hcudaDll,"MIPCuttingPlane");//获得函数指针
// 		if (dllFun)
// 		{
...
void SeVisualAPR::InitCuttingPlane()
{
	Safe_DeleteVec(m_pInitCuttingPlanePoint);
	m_pInitCuttingPlanePoint = new Vector3D[m_nCuttingPlaneHeight * m_nCuttingPlaneWidth];
	for (int i = -m_nCuttingPlaneHeight/2; i < m_nCuttingPlaneHeight/2; i++)
	{
		for (int j = -m_nCuttingPlaneWidth/2; j < m_nCuttingPlaneWidth/2; j++)
		{
			int nNum = (i + m_nCuttingPlaneHeight/2)*m_nCuttingPlaneWidth + (j + m_nCuttingPlaneWidth/2);
			m_pInitCuttingPlanePoint[nNum].x = j;
			m_pInitCuttingPlanePoint[nNum].y = i;
			m_pInitCuttingPlanePoint[nNum].z = 0.0;
		}
	}
}
