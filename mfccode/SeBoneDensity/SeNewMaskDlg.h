...
public:
	...
	CSeNewMaskDlg(LONG* pHistogram, LONG lMaxNumber, int nMax = 4096, int nMin = -1000, int nMaxPos = 4096, int nMinPos = -1000, CWnd* pParent = NULL);
	virtual ~CSeNewMaskDlg();

// 对话框数据
	enum { IDD = IDD_DIALOG_NEW_MASK_BONE };
...
public:
	const int GetMin(){return m_nMinPos;}
	const int GetMax(){return m_nMaxPos;}
