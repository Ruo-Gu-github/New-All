...
static void ErodeMask(BYTE* mask, int width, int height, int iterations) {
...
static void DilateMask(BYTE* mask, int width, int height, int iterations) {
     if (iterations <= 0) return;
    BYTE* temp = new BYTE[width * height];
    for (int it = 0; it < iterations; ++it) {
        memcpy(temp, mask, width * height);
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            if (left[idx] == 255 && !visited[idx]) {
                ...
                double ratio = (double)count / (bboxArea > 0 ? bboxArea : 1);
...
std::pair<BYTE*, BYTE*> FatAnalysis::SeprateFat(
    const short* pData,
    int width,
    int height,
    int depth,
    ...
    if (nowPos < 0 || nowPos >= depth) nowPos = depth / 2;
    ...
    // 1. Process nowPos
    {
        const short* pSlice = pData + nowPos * width * height;
        BYTE* layerFatMask = new BYTE[width * height];
        memset(layerFatMask, 0, width * height);

        for (int j = 0; j < width * height; ++j)
            if (pSlice[j] >= minFat && pSlice[j] <= maxFat)
                layerFatMask[j] = 255;
    ...
    for (int z = nowPos - 1; z >= 0; --z) {
        const short* pSlice = pData + z * width * height;
        BYTE* layerFatMask = new BYTE[width * height];
        memset(layerFatMask, 0, width * height);
...
vector<BYTE*> FatAnalysis::SeprateLung(const short* pData, int width, int height, int depth, int minFat, int maxFat, int minLung, int maxLung, int minBone, int maxBone)
{
    LONGLONG totalSize = (LONGLONG)width * height * depth;
    BYTE* fatMask = new BYTE[totalSize];
    memset(fatMask, 0, totalSize);
    ...
    if (seedLeft != -1) {
        Fast3DFloodFill(lungMask, width, height, depth, seedLeft, centerY, maxZ);
    }
    if (seedRight != -1) {
        Fast3DFloodFill(lungMask, width, height, depth, seedRight, centerY, maxZ);
    }
    ...
    for (int z = startZ; z <= endZ; ++z) {
        ...
        for (int i = 0; i < width * height; ++i) {
           if (!(pSlice[i] >= minFat && pSlice[i] <= maxFat))
                layerFat[i] = 0;
    ...
    if (boneSeedUp >= 0)
        Fast3DFloodFill(boneMask, width, height, depth, centerX, boneSeedUp, maxZ);
    if (boneSeedDown >= 0)
        Fast3DFloodFill(boneMask, width, height, depth, centerX, boneSeedDown, maxZ);
    if (boneSeedLeft >= 0)
        Fast3DFloodFill(boneMask, width, height, depth, boneSeedLeft, centerY, maxZ);
    if (boneSeedRight >= 0)
        Fast3DFloodFill(boneMask, width, height, depth, boneSeedRight, centerY, maxZ);
...
#pragma omp parallel for
    for (int z = start; z < end; ++z) {
        ...
        hull.push_back(Point2D(minX, minY));
