...
Napi::Value RenderAPRSlice(const Napi::CallbackInfo& info) {
    ...
    if (!is3D) {
        ...
        // 3D 视图：优先使用 roi3d 的三向 APR 句柄；若不存在则回退到 ImageBrowser 的三向 APR。
        auto find3dTriplet = [&](const std::string& a, const std::string& c, const std::string& s) -> bool {
            auto itA = g_AprHandles.find(a);
            auto itC = g_AprHandles.find(c);
            auto itS = g_AprHandles.find(s);
        ...
        const std::string roiAx = sessionId + "_roi3d_axial";
        const std::string roiCo = sessionId + "_roi3d_coronal";
        const std::string roiSa = sessionId + "_roi3d_sagittal";
        const std::string ibAx = sessionId + "_axial";
        const std::string ibCo = sessionId + "_coronal";
        const std::string ibSa = sessionId + "_sagittal";

        if (!find3dTriplet(roiAx, roiCo, roiSa) && !find3dTriplet(ibAx, ibCo, ibSa)) {
            printf(
                "[RenderViewToImage] ERROR: APR handle not found for 3d keys (roi3d=%s/%s/%s, imagebrowser=%s/%s/%s)\n",
                roiAx.c_str(), roiCo.c_str(), roiSa.c_str(),
                ibAx.c_str(), ibCo.c_str(), ibSa.c_str()
            );
