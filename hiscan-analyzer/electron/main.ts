...
import { NativeHost, resolveNativeHostWorkerPath } from "./nativeHost";

// Native HWND embedding (OpenGL child windows) is frequently occluded by
// Chromium GPU compositing on Windows. Disabling HW acceleration makes
// HWND z-ordering behave predictably.
app.disableHardwareAcceleration();

// Dev environments in locked-down Windows profiles can fail creating Chromium caches
// (GpuCache / disk cache) under the default userData path, causing Electron to quit.
// Use a temp-based userData/cache path to keep dev running reliably.
try {
...
function fileUrlToPlatformPath(urlStr: string): string {
...
const __filename = fileUrlToPlatformPath(import.meta.url);
...
function createCropTracer() {
  ...
  const start = (tag: string, sessionId?: string, data?: unknown) => {
    const t0 = Date.now();
    void trace({
      ts: nowIso(),
      pid: process.pid,
    ...
    return async (
      endTag: string,
      more?: { data?: unknown; error?: string }
    ) => {
...
// Custom message from native OpenGL child windows for double-click
const OGL_DBLCLICK_MSG = 0x8000 + 0x233;
...
const windowLabWindows = new Map<
  string,
  { info: WindowLabInfo; win: BrowserWindow }
>();
...
type OpenGLAddon = {
  ...
  createStandaloneWindow: (width: number, height: number) => bigint;
  setRect: (
    hwnd: bigint,
    x: number,
    y: number,
  ...
  setTransparentInput: (hwnd: bigint, enabled: boolean) => boolean;
  setVisible: (hwnd: bigint, visible: boolean) => boolean;
  getWindowInfo?: (hwnd: bigint) => any;
  destroy: (hwnd: bigint) => boolean;
};

type ConsoleDllAddon = {
  // dicom
  loadDicomFolderInfo?: (folderPath: string) => any;

  // APR / visualization
  createAPRViews?: (
    sessionId: string,
    folderPath: string,
    progressCb?: (p: number, m: string) => void
  ) => any;
  createMPRViews?: (
    sessionId: string,
    folderPath: string,
  ...
  getSessionWindowLevel?: (sessionId: string) => any;
  ...
  getSessionProjectionMode?: (sessionId: string) => any;
  ...
  updateAPRCenter?: (sessionId: string, x: number, y: number, z: number) => any;
  updateAPRRotation?: (
    sessionId: string,
    angleX: number,
    angleY: number,
  ...
  setCrosshairVisible?: (sessionId: string, visible: boolean) => any;
  getAPRState?: (sessionId: string, viewOrWindowId?: string) => any;
  destroyAPRViews?: (sessionId: string) => any;
  destroyMPRViews?: (sessionId: string) => any;
  embedWindow?: (
    windowIdOrHwnd: string | number,
    parentHwndBuffer: Buffer,
    x: number,
    y: number,
    width: number,
  ...
  invalidateWindow?: (windowId: string) => void;
  set3DOrthogonalMode?: (windowId: string, enabled: boolean) => boolean;
  set3DMaskIsoSurface?: (windowId: string, enabled: boolean) => boolean;
  getGpuInfo?: (windowId: string) => any;
  ...
  createAPRViewsFromHis4d?: (sessionId: string, his4dPath: string) => any;
  his4dSetFrame?: (sessionId: string, frameIndex: number) => any;
  his4dGetSessionInfo?: (sessionId: string) => any;
  destroyAll3DWindows?: () => void;
  hideAllWindows?: () => void;
  showAllWindows?: () => void;
  ...
  startRenderLoop?: (targetFps: number) => boolean;
  stopRenderLoop?: () => void;
  setWindowToolType?: (windowId: string, toolType: number) => boolean;
  enableAPRCropBox?: (enable: boolean) => void;
  setAPRCropBox?: (width: number, height: number, depth: number) => void;
  setAPRCropBoxRange?: (
    xStart: number,
    xEnd: number,
    yStart: number,
  ...
  isAPRCropBoxEnabled?: () => boolean;
  getCompletedMeasurements?: () => any;
  getMeasurementProfile?: (
    sessionId: string,
    measurementId: number,
    maxPoints?: number
  ) => any;
  deleteMeasurement?: (measurementId: number) => boolean;
  getMeasurementRegionHistogram?: (
    sessionId: string,
    measurementId: number
  ) => any;
  updateMPRCenter?: (sessionId: string, x: number, y: number, z: number) => any;

  resetView?: (windowId: string) => boolean;

  set3DVramOptimized?: (windowId: string, enabled: boolean) => any;

  // mask / histogram (optional)
  getVolumeHistogram?: (sessionId: string) => any;
  getMaskStatistics?: (sessionId: string, maskId: number) => any;
  updatePreviewMask?: (
    sessionId: string,
    minT: number,
    maxT: number,
  ...
  clearPreviewMask?: (sessionId: string) => any;
  ...
  selectMaskForEditing?: (sessionId: string, maskId: number) => any;
  setMaskTool?: (maskTool: number) => any;
  getMaskTool?: () => any;
  setMaskBrushRadius?: (radius: number) => any;
  getMaskBrushRadius?: () => any;
  deleteMask?: (sessionId: string, maskId: number) => any;
  saveMasks?: (sessionId: string, folderPath: string, maskName: string) => any;
  loadMasks?: (sessionId: string, folderPath: string) => any;
  exportMaskToStl?: (
    sessionId: string,
    maskId: number,
    filepath: string,
  ...
  maskInverse?: (sessionId: string, maskId: number) => any;

  // bone metrics
  getVolumeSpacing?: (sessionId: string) => any;
  ...
  fatAnalyzeSeparateLung?: (sessionId: string, maskId: number) => any;
...
/**
 * Adapter that wraps a direct ConsoleDllAddon to expose an interface compatible
 * with the NativeHost class (invoke/invokeWithProgress). This lets us reuse
 * existing IPC handler code that expects NativeHost while actually calling the
 * addon synchronously in the main process.
 */
class DirectAddonAdapter {
  private listeners = new Set<() => void>();

  constructor(private addon: ConsoleDllAddon) {}
  ...
  invoke(method: string, ...args: any[]): Promise<any> {
    return new Promise((resolve, reject) => {
      try {
        const fn = (this.addon as any)[method];
  ...
  invokeWithProgress(
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        // Many visualization methods accept progressCallback as last arg
  ...
  dispose() {
    // Nothing to do for direct addon
  }
}

function loadConsoleDllAddon(): ConsoleDllAddon | null {
  ...
  // Ensure dependent DLLs can be resolved.
  // - Dev: DLLs live next to the addon in native/console-dll/build/Release
  // - Prod: we package all *.dll into resources/native
  try {
  ...
  try {
    return require(addonPath) as ConsoleDllAddon;
  } catch (e) {
...
function resolveConsoleDllAddonPath(): string | null {
  // Same probing logic as loadConsoleDllAddon(), but we do NOT require() it
  // in the main process. The addon is loaded inside a utility process so
  // a native crash doesn't take down the whole app.
  let printedProbeError = false;
  const addonPath = (() => {
    if (!IS_DEV) {
      return path.join(
  ...
  if (!addonPath) return null;
  console.log("[viz] Addon path (native host):", addonPath);
  return addonPath;
}

type ManagedSeriesInfo = {
  id: string;
  folderPath: string;
  seriesName: string;
  fileCount: number;
...
const managedSeries = new Map<string, ManagedSeriesInfo>();
...
function registerDicomHandlers(nativeHost: NativeHostLike | null) {
  ...
  ipcMain.handle("app:restart-renderer", async () => {
    try {
      ...
      // Best-effort cleanup to avoid stale native HWNDs capturing input after reload.
      try {
        if (nativeHost) {
      ...
      if (win && !win.isDestroyed()) {
        win.reload();
        // allow a bit of time before accepting another restart request
        setTimeout(() => {
  ...
  ipcMain.handle("app:save-stl-file", async (_event, defaultName?: string) => {
    const result = await dialog.showSaveDialog({
  ...
  ipcMain.handle("dicom:scan-folder", async (_event, folderPath: string) => {
    try {
      if (!nativeHost) {
      ...
      const validation = info?.validation ?? undefined;

      const windowWidth =
        typeof tags.windowWidth === "number"
          ? Number(tags.windowWidth)
          : undefined;
      const windowLevel =
        typeof tags.windowCenter === "number"
          ? Number(tags.windowCenter)
          : undefined;

      const thumbnailBase64 =
        tryToBuffer(info?.thumbnail)?.toString("base64") ?? "";

      const seriesId = makeId("series");
      const seriesInfo: ManagedSeriesInfo = {
        id: seriesId,
        folderPath,
  ...
  ipcMain.handle("dicom:remove-series", async (_event, seriesId: string) => {
...
function loadOpenGLAddon(): OpenGLAddon | null {
  const require = createRequire(import.meta.url);
  ...
  try {
    return require(addonPath) as OpenGLAddon;
  } catch (e) {
...
function createMainWindow() {
  ...
  // Disabled: OpenGL child double-click hook
  // hookOpenGLDblClick(win);
...
function tryToBuffer(value: unknown): Buffer | null {
  try {
    ...
    // Some Electron/Native bridges serialize buffers as { type: 'Buffer', data: [...] }
    const anyVal = value as any;
    if (
      anyVal &&
      typeof anyVal === "object" &&
      Array.isArray(anyVal.data) &&
      (anyVal.type === "Buffer" || anyVal.type === "Uint8Array")
    ) {
      return Buffer.from(anyVal.data);
    }

    // Array-like (best-effort)
    if (
      anyVal &&
      typeof anyVal === "object" &&
...
function tryParseHwnd(value: unknown): bigint | null {
  try {
...
function tryParseHwndFromPayload(payload: any): bigint | null {
  return tryParseHwnd(payload?.hwnd);
}

function hookOpenGLDblClick(target: BrowserWindow) {
  try {
    // Electron on Windows calls callback(wParam, lParam) with Buffer-ish values.
    // It does NOT pass an event object.
    target.hookWindowMessage(OGL_DBLCLICK_MSG, (wParam, lParam) => {
      const hwnd = tryParseHwnd(wParam) ?? tryParseHwnd(lParam);
      if (!hwnd) {
        const wb = tryToBuffer(wParam);
        const lb = tryToBuffer(lParam);
        console.warn("[ogl] dblclick: invalid hwnd param", {
          wParamType: typeof wParam,
          lParamType: typeof lParam,
          wParamCtor: (wParam as any)?.constructor?.name,
          lParamCtor: (lParam as any)?.constructor?.name,
          wParamIsBuffer: Buffer.isBuffer(wParam),
          lParamIsBuffer: Buffer.isBuffer(lParam),
          wParamHex: wb
            ? wb.subarray(0, Math.min(8, wb.length)).toString("hex")
            : undefined,
          lParamHex: lb
            ? lb.subarray(0, Math.min(8, lb.length)).toString("hex")
            : undefined,
        });
        return;
      }
      if (target.isDestroyed() || target.webContents.isDestroyed()) return;
      const hwndStr = BigInt.asUintN(64, hwnd).toString();
      target.webContents.send("ogl:dblclick", { hwnd: hwndStr });
    });
  } catch (e) {
    console.warn("[ogl] hookWindowMessage failed", e);
  }
}

function loadRendererForWindow(
  target: BrowserWindow,
  opts?: { tab?: string; autocreate?: boolean }
) {
  if (IS_DEV && VITE_DEV_SERVER_URL) {
    const url = new URL(VITE_DEV_SERVER_URL);
    if (opts?.tab) url.searchParams.set("tab", opts.tab);
    if (opts?.autocreate) url.searchParams.set("autocreate", "1");
    target.loadURL(url.toString());
    return;
  }

  const searchParams = new URLSearchParams();
  if (opts?.tab) searchParams.set("tab", opts.tab);
  if (opts?.autocreate) searchParams.set("autocreate", "1");
...
function createWindowLabWindow(type: WindowLabType) {
  const id = makeId(type);
  const title =
    type === "opengl" ? `OpenGL Window (${id})` : `Normal Window (${id})`;

  const w = new BrowserWindow({
    width: type === "opengl" ? 1000 : 800,
  ...
  // Disabled: OpenGL child double-click hook
  // if (type === "opengl") hookOpenGLDblClick(w);

  if (type === "opengl") {
    loadRendererForWindow(w, { tab: "openglTest", autocreate: true });
  } else {
    const html = `<!doctype html>
...
<body>
  ...
  const info: WindowLabInfo = { id, type, title };
...
function registerWindowLabHandlers() {
  ipcMain.handle(
    "winlab:create",
    (_event, payload: { type: WindowLabType }) => {
      try {
        const info = createWindowLabWindow(payload.type);
        return { success: true, window: info };
      } catch (e: any) {
        return { success: false, error: e?.message ?? "winlab:create failed" };
      }
    }
  );

  ipcMain.handle("winlab:list", () => {
    const list = Array.from(windowLabWindows.values()).map(({ info, win }) => {
  ...
  ipcMain.handle(
    ...
    (
      ...
      try {
        ...
        return { success: false, error: e?.message ?? "winlab:action failed" };
      }
    }
  );
}

function registerOpenGLHandlers(addon: OpenGLAddon | null) {
  const hwndsByWebContentsId = new Map<number, Set<bigint>>();

  function reassertVisibleForWebContentsId(webContentsId: number) {
    if (!addon) return;
    const set = hwndsByWebContentsId.get(webContentsId);
    if (!set || set.size === 0) return;
    for (const hwnd of set) {
      try {
        // Force SW_SHOWNA + top Z-order within the parent (see native SetVisible).
  ...
  function getContentOriginDip(sender: Electron.WebContents) {
  ...
  function destroyForWebContentsId(webContentsId: number) {
    const set = hwndsByWebContentsId.get(webContentsId);
    if (!set || set.size === 0) return;
  ...
  // When a window loses/gains focus, Chromium may reorder its internal HWND tree.
  // Re-assert visibility to avoid the OpenGL child getting occluded until next click.
  ...
  app.on("browser-window-blur", (_e, bw) => {
    try {
      // Keep the child visible even when the window is inactive.
      if (bw.isMinimized()) return;
      reassertVisibleForWebContentsId(bw.webContents.id);
    } catch {
      // ignore
  ...
  ipcMain.handle(
    ...
    (
      ...
      // Important: parent to the WebContents HWND so the child window isn't
      // occluded by Chromium's content window.
      const parent =
        (event.sender as any).getNativeWindowHandle?.() ??
        bw.getNativeWindowHandle();
      ...
      const set =
        hwndsByWebContentsId.get(event.sender.id) ?? new Set<bigint>();
      set.add(hwnd);
  ...
  // Preferred: client-relative coordinates (CSS px) -> main converts to screen DIPs.
  ipcMain.handle(
    "ogl:create-client",
    (
      event,
  ...
  ipcMain.handle("ogl:status", () => {
    return {
      ...
      note: !!addon
        ? "addon loaded"
        : "addon not loaded (check build:native:ogl and path)",
    };
  });
  ...
  ipcMain.handle(
    ...
    (
      ...
      const w = Math.max(100, Math.floor(payload?.width ?? 900));
      const h = Math.max(100, Math.floor(payload?.height ?? 650));
      const hwnd = addon.createStandaloneWindow(w, h);

      // Track so it gets destroyed if the renderer goes away.
      const set =
        hwndsByWebContentsId.get(event.sender.id) ?? new Set<bigint>();
      set.add(hwnd);
      hwndsByWebContentsId.set(event.sender.id, set);
  ...
  ipcMain.handle(
    ...
    (_event, payload: { hwnd: string; enabled: boolean }) => {
      if (!addon) return { success: false, error: "OpenGL addon not loaded" };
      const hwnd = tryParseHwndFromPayload(payload);
      if (!hwnd) return { success: false, error: "invalid hwnd" };
  ...
  ipcMain.handle(
    "ogl:set-visible",
    (_event, payload: { hwnd: string; visible: boolean }) => {
      if (!addon) return { success: false, error: "OpenGL addon not loaded" };
      const hwnd = tryParseHwndFromPayload(payload);
      if (!hwnd) return { success: false, error: "invalid hwnd" };
  ...
  ipcMain.handle("ogl:get-window-info", (_event, payload: { hwnd: string }) => {
    if (!addon) return { success: false, error: "OpenGL addon not loaded" };
    const hwnd = tryParseHwndFromPayload(payload);
    if (!hwnd) return { success: false, error: "invalid hwnd" };
    try {
      const info = (addon as any).getWindowInfo?.(hwnd);
  ...
  ipcMain.handle("ogl:destroy", (event, payload: { hwnd: string }) => {
    if (!addon) return { success: false, error: "OpenGL addon not loaded" };
    const hwnd = tryParseHwndFromPayload(payload);
    if (!hwnd) return { success: false, error: "invalid hwnd" };
    addon.destroy(hwnd);
...
function registerPopupHandlers() {
  ...
  // A regular BrowserWindow opener for feature windows.
  // Unlike `popup:open`, this avoids always-on-top / pop-up-menu behavior and
  // uses showInactive() by default to reduce focus fights with the OpenGL child HWND.
  ...
  ipcMain.handle(
    ...
    (
      ...
      const id = makeId("win");
      const title = payload?.title ?? `Window (${id})`;
      const showInactive = payload?.showInactive ?? true;
      const modal = payload?.modal ?? false;
...
<body>
  ...
  ipcMain.handle(
    ...
    (
      ...
      const title = payload?.title ?? `Popup (${id})`;
...
function registerDialogHandlers() {
  ...
  const dialogContextsByWebContentsId = new Map<number, DialogContext>();

  type ConfirmContext = {
    resolve: (ok: boolean) => void;
    resolved: boolean;
  };
  const confirmContextsByWebContentsId = new Map<number, ConfirmContext>();
  ...
  ipcMain.handle(
    "dialog:open",
    async (event, dialogType: string, params: any) => {
      ...
      // IMPORTANT: after a BrowserWindow is closed, accessing dialogWindow.webContents
      // can throw "Object has been destroyed". Capture the id now and reuse it.
      ...
      try {
        ...
        // Notify the parent renderer BEFORE showing/focusing the dialog.
        // Otherwise the main window may blur first (show/focus), pause the render loop,
        // and cause embedded native HWNDs to get hidden/lose mouse routing.
        try {
          const parentWc = parent?.webContents;
          parentWc?.send("dialog:opened", {
      ...
      dialogWindow.on("closed", () => {
        ...
        const parentWc =
          parent?.webContents ??
          (c?.parentWebContentsId != null
            ? webContents.fromId(c.parentWebContentsId)
            : null);
        parentWc?.send("dialog:closed", {
          webContentsId: dialogWebContentsId,
          dialogType: c?.dialogType,
        });
      });
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const title = payload?.title ?? "确认";
      const message = payload?.message ?? "";
      const confirmButtonText = payload?.confirmButtonText ?? "确定";
      const cancelButtonText = payload?.cancelButtonText ?? "取消";
      const showInactive = payload?.showInactive ?? true;
      ...
      // If the window closed immediately (rare), just return.
...
<body>
  ...
  <div class="btns">
    ...
    <button class="btn primary" type="button" onclick="window.ipcRenderer?.send?.('dialog-confirm:response', { ok: true })">${escapeHtml(
      ...
      // Load after creating the promise wiring above.
      w.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(html)}`);
  ...
  ipcMain.on("dialog-confirm:response", (event, payload: { ok: boolean }) => {
    const wcId = event.sender.id;
    const ctx = confirmContextsByWebContentsId.get(wcId);
    if (!ctx || ctx.resolved) return;
    ctx.resolved = true;
    confirmContextsByWebContentsId.delete(wcId);
    ctx.resolve(!!payload?.ok);

    const dialogWin = BrowserWindow.fromWebContents(event.sender);
    if (dialogWin && !dialogWin.isDestroyed()) dialogWin.close();
  });

  // Renderer-side dialogs may register IPC listeners after did-finish-load.
  // Use a ready handshake to reliably deliver init data.
  ...
  ipcMain.on("dialog:close", (event, result: any) => {
    const dialogWin = BrowserWindow.fromWebContents(event.sender);
    ...
    parentWc?.send("dialog:result", taggedResult);

    dialogWin.close();
  });
}

function registerVisualizationHandlers(nativeHost: NativeHostLike | null) {
  const cropTrace = createCropTracer();
  const cropErrorMessage = (e: unknown) => {
  ...
  ipcMain.handle(
    "viz:create-apr",
    async (event, sessionId: string, folderPath: string) => {
      try {
        if (!nativeHost) {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        // Back-compat: old signature was (sessionId, folderPath, selection, width, height)
        // New signature is (sessionId, folderPath, selection, hwndMapOrWidth, width, height)
        let hwndMap: Record<string, string> | undefined;
        let resolvedWidth: number | undefined = width;
        let resolvedHeight: number | undefined = height;

        if (
          windowHwndsOrWidth &&
          typeof windowHwndsOrWidth === "object" &&
          !Array.isArray(windowHwndsOrWidth)
        ) {
          hwndMap = windowHwndsOrWidth as Record<string, string>;
        ...
        const reqW =
          Number.isFinite(resolvedWidth as any) && (resolvedWidth as number) > 0
            ? (resolvedWidth as number)
            : 1024;
        const reqH =
          Number.isFinite(resolvedHeight as any) &&
          (resolvedHeight as number) > 0
            ? (resolvedHeight as number)
            : 1024;

        const expandSelection = (sel: string[] | undefined): string[] => {
        ...
        const rgbaToBgraInPlace = (buf: Buffer) => {
          for (let i = 0; i + 3 < buf.length; i += 4) {
            const r = buf[i];
            buf[i] = buf[i + 2];
            buf[i + 2] = r;
            // Ensure opaque output; many sources produce alpha=0.
            buf[i + 3] = 0xff;
          }
        };

        const getHwndForView = (viewName: string): string | null => {
          if (!hwndMap) return null;
          const direct = hwndMap[viewName] ?? hwndMap[viewName.toLowerCase()];
          if (direct) return String(direct);

          // Allow callers to pass windowId->hwnd mapping as well.
          const byId = hwndMap[String(viewName)];
          return byId ? String(byId) : null;
        };

        const normalizeBgraToSize = (
          bgra: Buffer,
          srcW: number,
          srcH: number,
          dstW: number,
        ...
        const renderViewToBgra = async (
          viewName: string,
          w: number,
          h: number
        ): Promise<
          | { ok: true; bgra: Buffer; width: number; height: number }
          | { ok: false; error: string }
        > => {
          try {
            // Prefer HWND screenshot when provided (especially for 3D / quad:3d).
            ...
            if (hwnd) {
              ...
              const r: any = await nativeHost.invoke(
                "captureWindowByHWND",
                hwnd
              );
              outW = Number(r?.width);
              outH = Number(r?.height);
              pixelData = r?.data;
            } else {
              // Fallback to APR slice render (legacy path). NOTE: 3d does not have an APR handle.
              if (viewName === "3d") {
                return {
                  ok: false,
                  error:
                    "Missing 3D HWND for screenshot (cannot use renderAPRSlice for 3d)",
                };
              }
              ...
              if (!r?.success) {
                return {
                  ok: false,
                  error: r?.error || `renderAPRSlice failed for ${viewName}`,
                };
              }

              outW = Number(r?.width ?? w);
              outH = Number(r?.height ?? h);
            ...
            const rgba = Buffer.isBuffer(pixelData)
              ? Buffer.from(pixelData)
              : Buffer.from(pixelData as Uint8Array);
        ...
        if (viewsSet.has("quad")) {
          ...
          const order: Array<{ key: string; x: number; y: number }> = [
            { key: "axial", x: 0, y: 0 },
            { key: "coronal", x: 1, y: 0 },
          ...
          for (const item of order) {
            ...
            if (!rr.ok) {
              ...
              // 继续处理其他tile，但标记quad为失败
              quadOk = false;
              continue;
            }
            // If native returns unexpected size, skip to avoid corrupting layout.
          ...
          // 即使部分tile失败，如果有至少2个成功的tile，也尝试保存
  ...
  ipcMain.handle(
    "viz:update-center",
    async (_event, sessionId: string, x: number, y: number, z: number) => {
      try {
        if (!nativeHost) {
          return { success: false, error: "Visualization API not available" };
        }
  ...
  ipcMain.handle(
    ...
    async (_event, sessionId: string, visible: boolean) => {
      try {
        if (!nativeHost) {
          return { success: false, error: "Visualization API not available" };
        }
  ...
  ipcMain.handle(
    "viz:get-apr-state",
    async (_event, sessionId: string, viewOrWindowId?: string) => {
      try {
        if (!nativeHost) {
          return { success: false, error: "Visualization API not available" };
        }
  ...
  ipcMain.handle("viz:get-window-level", async (_event, sessionId: string) => {
    try {
      if (!nativeHost) {
        return { success: false, error: "Visualization API not available" };
      }
  ...
  ipcMain.handle(
    ...
    async (_event, sessionId: string, mode: number, thickness: number) => {
      try {
        if (!nativeHost) {
          return { success: false, error: "Visualization API not available" };
        }
  ...
  // 裁切相关 IPC handlers
  ipcMain.handle("viz:crop-volume", async (_event, sessionId: string) => {
    ...
    try {
      ...
      const cropSettings = await nativeHost
        .invoke("getAPRCropSettings", sessionId)
        .catch((e: any) => ({ error: e?.message ?? String(e) }));
      const aprStateAxial = await nativeHost
        .invoke("getAPRState", sessionId, "axial")
        .catch((e: any) => ({ error: e?.message ?? String(e) }));
      ...
      await end("viz:crop-volume", { error: e?.message ?? String(e) });
      return { success: false, error: e?.message ?? String(e) };
    }
  });
    ...
    async (_event, sessionId: string) => {
      ...
      try {
        ...
        const cropSettings = await nativeHost
          .invoke("getAPRCropSettings", sessionId)
          .catch((e: any) => ({ error: e?.message ?? String(e) }));
        const aprStateAxialBefore = await nativeHost
          .invoke("getAPRState", sessionId, "axial")
          .catch((e: any) => ({ error: e?.message ?? String(e) }));
        await cropTrace.trace({
          ts: new Date().toISOString(),
        ...
        const aprStateAxialAfter = await nativeHost
          .invoke("getAPRState", sessionId, "axial")
          .catch((e: any) => ({ error: e?.message ?? String(e) }));
        await cropTrace.trace({
          ts: new Date().toISOString(),
  ...
  // 裁切设置相关 IPC handlers
  ipcMain.handle("viz:set-crop-shape", async (_event, shape: number) => {
    const end = cropTrace.start("viz:set-crop-shape", undefined, { shape });
    try {
      if (!nativeHost) {
      ...
      const settings = await nativeHost
        .invoke("getAPRCropSettings")
        .catch((e: any) => ({ error: e?.message ?? String(e) }));
      await end("viz:set-crop-shape", { data: { success: true, settings } });
      return { success: true };
    } catch (e: any) {
      console.error("[viz:set-crop-shape] Error:", e);
      await end("viz:set-crop-shape", { error: e?.message ?? String(e) });
      return { success: false, error: e?.message ?? String(e) };
    }
  });
  ...
  ipcMain.handle("viz:set-crop-box-size", async (_event, ...args: any[]) => {
    const sessionId =
      typeof args?.[0] === "string" ? (args[0] as string) : undefined;
    const offset = sessionId ? 1 : 0;
    const sizeX = Number(args?.[0 + offset] ?? 0);
    const sizeY = Number(args?.[1 + offset] ?? 0);
    const sizeZ = Number(args?.[2 + offset] ?? 0);
    const volumeWidth = Number(args?.[3 + offset] ?? 0);
    const volumeHeight = Number(args?.[4 + offset] ?? 0);
    const volumeDepth = Number(args?.[5 + offset] ?? 0);
    ...
    try {
      ...
      await end("viz:set-crop-box-size", { error: e?.message ?? String(e) });
      return { success: false, error: e?.message ?? String(e) };
    }
  });
  ...
  ipcMain.handle(
    ...
    async (_event, sessionId?: string) => {
      try {
        ...
        return (
          (await nativeHost.invoke("getAPRCropSettings", sessionId ?? "")) ?? {
            shape: 0,
            cylinderDirection: 0,
            enabled: false,
            cropBox: {},
  ...
  ipcMain.handle("viz:destroy-apr", async (_event, sessionId: string) => {
    try {
      if (!nativeHost) {
        return { success: false, error: "Visualization API not available" };
      }
  ...
  ipcMain.handle("viz:destroy-mpr", async (_event, sessionId: string) => {
    try {
      if (!nativeHost) {
        return { success: false, error: "Visualization API not available" };
      }
  ...
  ipcMain.handle("viz:invalidate-window", async (_event, windowId: string) => {
    try {
      if (!nativeHost) return;
  ...
  ipcMain.handle("viz:reset-view", async (_event, windowId: string) => {
    try {
  ...
  ipcMain.handle(
    ...
    async (_event, windowId: string, enabled: boolean) => {
      try {
        if (!nativeHost) return false;
        return (
          (await nativeHost.invoke("set3DOrthogonalMode", windowId, enabled)) ??
          false
        );
      } catch (e) {
  ...
  ipcMain.handle(
    ...
    async (_event, windowId: string, enabled: boolean) => {
      try {
        ...
        if (!nativeHost) return false;
        return (
          (await nativeHost.invoke("set3DVramOptimized", windowId, enabled)) ??
          false
        );
      } catch (e) {
  ...
  ipcMain.handle(
    ...
    async (_event, windowId: string, enabled: boolean) => {
      try {
        if (!nativeHost) return false;
        return (
          (await nativeHost.invoke("set3DMaskIsoSurface", windowId, enabled)) ??
          false
        );
      } catch (e) {
  ...
  ipcMain.handle("viz:get-gpu-info", async (_event, windowId: string) => {
    try {
      if (!nativeHost) {
        return { success: false, error: "Visualization API not available" };
      }
  ...
  ipcMain.handle(
    ...
    async (_event, sessionId: string, his4dPath: string) => {
      try {
        if (!nativeHost) {
          return { success: false, error: "Visualization API not available" };
        }
  ...
  ipcMain.handle(
    "viz:his4d-set-frame",
    async (_event, sessionId: string, frameIndex: number) => {
      try {
        if (!nativeHost) {
          return { success: false, error: "Visualization API not available" };
        }
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        // Use embedWindow to keep parenting/DPI and mouse routing consistent after resize.
        if (!nativeHost) return false;
  ...
  // NOTE: Multiple tabs/components may call startRenderLoop() on mount/activate.
  // Treat it as idempotent to avoid noisy errors when switching tabs.
  let isRenderLoopRunning = false;
  let renderLoopTargetFps: number | undefined;
  ...
  const isAddonMissingMethodError = (e: unknown, method: string) => {
    const msg = getErrorMessage(e);
    return msg.includes(`Method ${method} not found on addon`);
  };

  ipcMain.handle("viz:start-render-loop", async (_event, targetFPS: number) => {
    try {
      if (!nativeHost) return false;
      if (isRenderLoopRunning) {
        // If caller asks for the same fps, just treat as success.
        if (
          renderLoopTargetFps === undefined ||
          renderLoopTargetFps === targetFPS
        ) {
          renderLoopTargetFps = targetFPS;
          return true;
        }
        // If a different fps is requested, best-effort restart.
        try {
      ...
      const ok =
        (await nativeHost.invoke("startRenderLoop", targetFPS)) ?? false;
      isRenderLoopRunning = !!ok;
      if (ok) renderLoopTargetFps = targetFPS;
      return ok;
    } catch (e) {
      const msg = getErrorMessage(e);
      // Native side may throw when already running; treat as success.
  ...
  ipcMain.handle(
    ...
    async (_event, windowId: string) => {
      try {
        if (!nativeHost) return false;
        return (
          (await nativeHost.invoke("refreshWindowZOrder", windowId)) ?? false
        );
      } catch (e) {
  ...
  ipcMain.handle("viz:raise-window", async (_event, windowId: string) => {
    try {
      if (!nativeHost) return false;
      return (await nativeHost.invoke("raiseWindow", windowId)) ?? false;
    } catch (e) {
      // Older/native-mismatched addons may not export raiseWindow yet.
      if (isAddonMissingMethodError(e, "raiseWindow")) {
        ...
        try {
          // Best-effort: refresh z-order so the embedded HWND is visible.
          await nativeHost
            .invoke("refreshWindowZOrder", windowId)
            .catch(() => {});
  ...
  // Keep embedded native windows visible across focus changes.
  // IMPORTANT: do not fight minimize; minimized windows should remain hidden.
  app.on("browser-window-focus", (_e, bw) => {
    try {
      if (bw.isMinimized()) return;
      void nativeHost?.invoke("refreshAllWindowsZOrder");
    } catch {
      // ignore
    }
  });
  ...
  ipcMain.handle(
    ...
    async (_event, windowId: string, toolType: number) => {
      try {
        if (!nativeHost) return false;
        return (
          (await nativeHost.invoke("setWindowToolType", windowId, toolType)) ??
          false
        );
      } catch (e) {
  ...
  ipcMain.handle(
    ...
    async (_event, windowId: string, visible: boolean) => {
      const end = cropTrace.start(
        "viz:set-window-crop-box-visible",
        undefined,
        {
  ...
  ipcMain.handle("viz:enable-apr-crop-box", async (_event, ...args: any[]) => {
    const sessionId =
      typeof args?.[0] === "string" ? (args[0] as string) : undefined;
    const enable = Boolean(sessionId ? args?.[1] : args?.[0]);
  ...
  ipcMain.handle("viz:set-apr-crop-box", async (_event, ...args: any[]) => {
    const sessionId =
      typeof args?.[0] === "string" ? (args[0] as string) : undefined;
    const offset = sessionId ? 1 : 0;
    const width = Number(args?.[0 + offset] ?? 0);
    const height = Number(args?.[1 + offset] ?? 0);
    const depth = Number(args?.[2 + offset] ?? 0);
  ...
  ipcMain.handle(
    ...
    async (_event, ...args: any[]) => {
      const sessionId =
        typeof args?.[0] === "string" ? (args[0] as string) : undefined;
      const offset = sessionId ? 1 : 0;
      const xStart = Number(args?.[0 + offset]);
      const xEnd = Number(args?.[1 + offset]);
      const yStart = Number(args?.[2 + offset]);
      const yEnd = Number(args?.[3 + offset]);
      const zStart = Number(args?.[4 + offset]);
      const zEnd = Number(args?.[5 + offset]);
  ...
  ipcMain.handle("viz:get-apr-crop-box", async (_event, sessionId?: string) => {
    try {
      if (!nativeHost) return {};
  ...
  ipcMain.handle(
    ...
    async (_event, sessionId?: string) => {
      try {
        if (!nativeHost) return false;
        return (
          (await nativeHost.invoke("isAPRCropBoxEnabled", sessionId ?? "")) ??
          false
        );
      } catch (e) {
  ...
  ipcMain.handle(
    ...
    async (_event, measurementId: number) => {
      try {
        if (!nativeHost) return false;
        return (
          (await nativeHost.invoke("deleteMeasurement", measurementId)) ?? false
        );
      } catch (e) {
  ...
  ipcMain.handle(
    ...
    async (_event, sessionId: string, measurementId: number) => {
      try {
        if (!nativeHost) {
          return { success: false, error: "Native host unavailable" };
        }
  ...
  ipcMain.handle(
    ...
    async (_event, sessionId: string, maskId: number) => {
      try {
        if (!nativeHost)
          return { success: false, error: "Native host unavailable" };
  ...
  ipcMain.handle(
    ...
    async (_event, sessionId: string, maskId: number, roiMaskId?: number) => {
      try {
        if (!nativeHost)
          return { success: false, error: "Native host unavailable" };
        return await nativeHost.invoke(
  ...
  ipcMain.handle(
    "viz:save-masks",
    async (_event, sessionId: string, folderPath: string, maskName: string) => {
      try {
        if (!nativeHost)
          return { success: false, error: "Native host unavailable" };
        return await nativeHost.invoke(
  ...
  ipcMain.handle(
    "viz:load-masks",
    async (_event, sessionId: string, folderPath: string) => {
      try {
        if (!nativeHost)
          return { success: false, error: "Native host unavailable" };
  ...
  ipcMain.handle(
    ...
    async (_event, sessionId: string, arg1: any, arg2: any) => {
      try {
        if (!nativeHost)
          return { success: false, error: "Native host unavailable" };

        // Pass arguments through (handled by N-API wrapper: (sessionId, maskBuffer) OR (sessionId, minTh, maxTh))
        const result = await nativeHost.invoke(
          "vascularAnalyzeCompute",
          sessionId,
          arg1,
  ...
  ipcMain.handle(
    ...
    async (_event, maskBuffer: Uint8Array, w: number, h: number, d: number) => {
      try {
        if (!nativeHost)
          return { success: false, error: "Native host unavailable" };
...
app.whenReady().then(() => {
  // Remove default application menu (File/Edit/View...) so frameless windows
  // don't show a native menu bar.
  Menu.setApplicationMenu(null);

  const addon = loadOpenGLAddon();

  // Load console DLL addon directly in main process.
  // We previously used NativeHost (utility process) but Win32 window creation
  // requires a message pump which utility processes don't have, causing hangs.
  const consoleDllAddon = loadConsoleDllAddon();

  // Wrap the addon in a DirectAddonAdapter so existing handler code (which expects
  // NativeHost-style invoke/invokeWithProgress) works unchanged.
  const nativeHost = consoleDllAddon
    ? new DirectAddonAdapter(consoleDllAddon)
    : null;

  // Note: With DirectAddonAdapter, native crashes will crash the main process.
  // This is acceptable as the alternative (utility process) doesn't support Win32 windows.
