...
function registerVisualizationHandlers(nativeHost: NativeHostLike | null) {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        const reqW =
          Number.isFinite(width as any) && (width as number) > 0
            ? (width as number)
            : 1024;
        const reqH =
          Number.isFinite(height as any) && (height as number) > 0
            ? (height as number)
            : 1024;
        ...
        > => {
          try {
            ...
            const outW = Number(r?.width ?? w);
            const outH = Number(r?.height ?? h);
            const pixelData = r?.pixelData;
  ...
  ipcMain.handle("viz:get-crop-settings", async () => {
    try {
      ...
      return (
        (await nativeHost.invoke("getAPRCropSettings")) ?? {
          shape: 0,
          cylinderDirection: 0,
          enabled: false,
          cropBox: {},
  ...
  ipcMain.handle("viz:enable-apr-crop-box", async (_event, enable: boolean) => {
    try {
      if (!nativeHost) return false;
  ...
  ipcMain.handle(
    ...
    async (_event, width: number, height: number, depth: number) => {
      try {
        if (!nativeHost) return false;
